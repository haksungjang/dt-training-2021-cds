# -*- coding: utf-8 -*-
"""CDS리그도전_0731_V2.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1z4fPhVIXcxvNCNkh8UeELxKdEEhhgfDu
...
ds_coin = "coin.csv"
df_coin = pd.read_csv(ds_coin, parse_dates=['registerdate', 'coinexpiredate'], infer_datetime_format=True)
...
# haksung : coin 데이터 중 유의미한 값을 df_service에 포함하자. 
df_coin_target = df_coin.drop(["registerdate", "currencytypeid", "discountamount", "productname"], axis=1)
df_coin_target = df_coin_target.drop_duplicates(["uno"], keep="last")
df_coin_target.rename(columns={'pgamount':'coinpgamount'}, inplace=True)
df_coin_target.rename(columns={'productcode':'coinproductcode'}, inplace=True)

df_service = pd.merge(df_service, df_coin_target, on='uno', how='left')

# haksung : registerdate보다 4주 지난 시점보다 coinexpiredate이 더 긴지 여부를 판단하여 coinexpired 컬럼에 추가하자. 
df_service['coinexpired'] = np.where( df_service.coinexpiredate.dt.date > (df_service.registerdate + pd.DateOffset(weeks=4)).dt.date, 0, 1 )
...
# haksung : coinpgamount 값 및 분포 확인. 마이너스 값 등 이상치가 존재하나, 일단, 그냥 두자. 
print('coinpgamount = ', sorted(df_service.coinpgamount.unique().tolist()))
df_service.coinpgamount.value_counts()
...
# haksung : device_list_cnt - 사용 디바이스의 갯수도 추가해보자. 
device_list_cnt = []
for i in device_list:
  device_list_cnt.append(len(i))
...
# haksung : type_list_cnt - 시청권한 갯수도 추가해보자. 
type_list_cnt = []
for i in type_list:
  type_list_cnt.append(len(i))
...
pcode_struct = {'productcode': idx_pcode,
                ...
                'PC_DEVICE_CNT': device_list_cnt,
                'PC_TYPE': type_list,
                'PC_TYPE_CNT': type_list_cnt,
                'PC_IMAGE': iamge_list,
...
### haksung : 고객별 최고 선호 채널타입, 섹션, 디바이스를 추출해보자. 
#   (16) 고객별 선호 채널 타입
#   (17) 고객별 선호 섹션
#   (18) 고객별 선호 디바이스
###
...
# haksung 
# (16) 고객별 선호 채널 타입
df_common_channeltype = df_bookmark[['uno','channeltype']]
df_common_channeltype = df_common_channeltype.groupby('uno')['channeltype'].agg(**{
    'CH_COMMON':lambda x:list(x.mode())
}).reset_index()
df_common_channeltype.CH_COMMON = df_common_channeltype.CH_COMMON.apply(lambda x:x[0])

# (17) 고객별 선호 섹션
df_common_section = df_bookmark[['uno', 'section']]
df_common_section = df_common_section.groupby('uno')['section'].agg(**{
    'SEC_COMMON':lambda x:list(x.mode())
}).reset_index()
df_common_section.SEC_COMMON = df_common_section.SEC_COMMON.apply(', '.join)

# (18) 고객별 선호 디바이스
df_common_device = df_bookmark[['uno', 'devicetype']]
df_common_device = df_common_device.groupby('uno')['devicetype'].agg(**{
    'DEV_COMMON':lambda x:list(x.mode())
}).reset_index()
df_common_device.DEV_COMMON = df_common_device.DEV_COMMON.apply(lambda x:x[0])
...
# haksung
print('df_common_channeltype = ', df_common_channeltype.shape[0])
print('df_common_section = ', df_common_section.shape[0])
print('df_common_device = ', df_common_device.shape[0])
...
df_svc_target = pd.merge(df_svc_target, df_common_channeltype, on='uno', how='left')
df_svc_target = pd.merge(df_svc_target, df_common_section, on='uno', how='left')
df_svc_target = pd.merge(df_svc_target, df_common_device, on='uno', how='left')
...
df_svc_target = df_svc_target.astype({'PC_DEVICE_CNT':'float64', 'PC_TYPE_CNT':'float64'}) # haksung
df_svc_target = df_svc_target.astype({'CH_COMMON':'str', 'DEV_COMMON':'str', 'SEC_COMMON':'str'}) # haksung
...
col_lst.remove('coinexpiredate')    # haksung
col_cat_lst = []
...
plt.figure(figsize=(36,20)) # value가 보여지도록 figsize 증가 by haksung
...
# Train/Test 데이터 분리 
X = df_T.drop(["uno","registerdate","enddate","productcode","Repurchase","coinexpiredate", "CHURN"], axis=1) #haksung : coinexpiredate
y = df_T["CHURN"]
...
#                        n_jobs=-1, random_state=SEED, importance_type='gain')
    ...
    lgbm = LGBMClassifier(objective='binary',
                          ...
                          learning_rate=0.1,
                          max_depth=78, #67, #22,
                          subsample=0.9819, #0.7921, #0.8928,
                          subsample_freq=3,
                          num_leaves=64, #35, #10,
                          colsample_bytree=0.4349, #0.4591, #0.6375,
                          reg_alpha=0,
                          reg_lambda=2,
                          min_split_gain=0,
                          min_child_samples=10,
                          min_child_weight=42.08, #67.47, #6.582,
                          scale_pos_weight=2.751,
                          n_jobs=-1,
                          random_state=SEED,
...
# Coin 파일 읽기
ds_coin = "coin.csv"
df_P_coin = pd.read_csv(ds_coin, parse_dates=['registerdate', 'coinexpiredate'], infer_datetime_format=True)
...
# haksung : coin 데이터 중 유의미한 값을 df_service에 포함하자. 
df_P_coin_target = df_P_coin.drop(["registerdate", "currencytypeid", "discountamount", "productname"], axis=1)
df_P_coin_target = df_P_coin_target.drop_duplicates(["uno"], keep="last")
df_P_coin_target.rename(columns={'pgamount':'coinpgamount'}, inplace=True)
df_P_coin_target.rename(columns={'productcode':'coinproductcode'}, inplace=True)
df_P_service = pd.merge(df_P_service, df_P_coin_target, on='uno', how='left')

# haksung : registerdate보다 4주 지난 시점보다 coinexpiredate이 더 긴지 여부를 판단하여 coinexpired 컬럼에 추가하자. 
df_P_service['coinexpired'] = np.where( df_P_service.coinexpiredate.dt.date > (df_P_service.registerdate + pd.DateOffset(weeks=4)).dt.date, 0, 1 )
...
# (16) 고객별 선호 채널 타입
df_P_common_channeltype = df_P_bookmark[['uno','channeltype']]
df_P_common_channeltype = df_P_common_channeltype.groupby('uno')['channeltype'].agg(**{
    'CH_COMMON':lambda x:list(x.mode())
}).reset_index()
df_P_common_channeltype.CH_COMMON = df_P_common_channeltype.CH_COMMON.apply(lambda x:x[0])

# (17) 고객별 선호 섹션
df_P_common_section = df_P_bookmark[['uno', 'section']]
df_P_common_section = df_P_common_section.groupby('uno')['section'].agg(**{
    'SEC_COMMON':lambda x:list(x.mode())
}).reset_index()
df_P_common_section.SEC_COMMON = df_P_common_section.SEC_COMMON.apply(', '.join)

# (18) 고객별 선호 디바이스
df_P_common_device = df_P_bookmark[['uno', 'devicetype']]
df_P_common_device = df_P_common_device.groupby('uno')['devicetype'].agg(**{
    'DEV_COMMON':lambda x:list(x.mode())
}).reset_index()
df_P_common_device.DEV_COMMON = df_P_common_device.DEV_COMMON.apply(lambda x:x[0])
...
df_P_svc_target = pd.merge(df_P_svc_target, df_P_common_channeltype, on='uno', how='left')
df_P_svc_target = pd.merge(df_P_svc_target, df_P_common_section, on='uno', how='left')
df_P_svc_target = pd.merge(df_P_svc_target, df_P_common_device, on='uno', how='left')
...
df_P_svc_target = df_P_svc_target.astype({'CH_COMMON':'str', 'DEV_COMMON':'str', 'SEC_COMMON':'str'}) # haksung
...
# 예측 모델 입력을 위한 최종 데이터
X_predict = df_P.drop(["uno","registerdate","enddate","productcode","Repurchase","coinexpiredate"], axis=1)
...
# 답안지 제출 파일 생성하기
ds_answer_sheet = "CDS_submission_Beyond AI for LOVE_11차.csv"
